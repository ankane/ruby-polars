...
class DocsTest < Minitest::Test
  ...
  def assert_examples(method, cls)
    # requires files
    return if [:read_csv_batched, :sink_parquet, :sink_ipc, :sink_csv, :sink_ndjson, :sink_batches, :sink_delta].include?(method.name)
    ...
    method.tags(:example).each do |example|
      ...
      begin
        ...
        if todo
          ...
          output = output.gsub("\t", "        ")
          assert_equal expected, output, "Example output (#{method.name})"
        elsif !([:initialize, :lit, :plot, :tree_format, :explain, :lazy].include?(method.name) || method.name.start_with?("write_") || cls == Polars::Selectors || lines.last.include?(" = "))
